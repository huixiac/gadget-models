...
title: Gadget assessment of Greater silver smelt *Argentina silus* in Icelandic and
  ...
  estimation and reference points
...
knitr::opts_chunk$set(echo = TRUE,  warning = FALSE, message = FALSE)
  ...
  #  fit_age1<- gadget.fit( gd = '/home/unu/huixia/gadget project/lemon gadget/45-base',
  #                    recruitment_step_age	= tibble(stock = 'lemimm', age = 1, step = 1), 
  #                    f.age.range = data.frame(stock = c('lemmat', 'lemimm'), age.min = c(1,1), age.max = c(16,16)),
   #                   wgts = 'WGTS')
    fit<- gadget.fit( gd = '/home/unu/huixia/gadget project/lemon gadget/45-base',
                          recruitment_step_age	= tibble(stock = 'lemimm', age = 1, step = 1), 
                          f.age.range = data.frame(stock = c('lemmat', 'lemimm'), age.min = c(1,1), age.max = c(16,16)),
                    wgts = 'WGTS')
  ...
  load('/home/unu/huixia/gadget project/lemon gadget/45-base/bootfit.Rdata')
  ...
  load('/home/unu/huixia/gadget project/lemon gadget/45-base/retroFit.Rdata')

  
#  load('/net/hafkaldi/export/home/haf/pamela/19-gss/exploratory_models/gadget/01-new_ass_no_igfs_mt_s5/res.Rdata')
#  load('/net/hafkaldi.hafro.is/export/home/haf/pamela/19-gss/exploratory_models/gadget/01-new_ass_no_igfs_mt_s5/res.Rdata')  

  
#load('/net/hafkaldi.hafro.is/export/home/haf/pamela/19-gss/exploratory_models/gadget/01-new_ass_no_igfs_mt_s5/ref_points.Rdata')

```

#### Bootstrap-estimated uncertainty

The bootstrap replicates generally showed parameter estimation with a central tendency (Figures 1 - 5). As would be expected, many of the growth parameters are highly correlated (Figure 2). IUncertainty is high but the base model follows the median results of the bootstrap runs in both the model results and fit to the model indices (Figures 6 - 9). More to be written here about the bootstrap figures here...
...
bootfit.igfs$res.by.year %>%
  ...
  geom_line(data=fit$res.by.year %>% filter(stock=='lemimm', year > 1999),aes(y=recruitment/1e6,group=1),col='red') +
...
for $B_{lim}$ can be calculated based on the inverse of the standard factor, $e^{\sigma * 1.645}$ where $\sigma$ is 0.2, used for calculating $B_{pa}$ from $B_{lim}$. Therefore, a proxy for $B_{lim}$ could be set at $B_{pa}/e^{1.645*0.2} = $`r  round(bpa, 2)` $/1.39 = $ `r blim` kt. 
...
Figure 15 shows the evolution of catches, SSB and fishing mortality for select values of $F$ are shown. The equilibrium yield curve is shown in figure
14, where the maximum average yield, under the recruitment
assumptions, is roughly 12.5 thousand tons with a 95% interval of for the yield ranging 0 to almost 16 thousand tons. $F_{msy}$ was estimated to be `r round(f.msy,2)`, corresponding with $H_{msy}$ of `r round(hr_msy,2)`. 
...
# gridExtra::grid.arrange(yield.plot,ssbmsy,ncol=2)
  ...
  knitr::include_graphics('/net/hafkaldi.hafro.is/export/home/haf/pamela/19-gss/exploratory_models/gadget/01-new_ass_no_igfs_mt_s5/yield_plot.png', dpi = 100)
...
```{r exampleFs, eval = FALSE,echo=FALSE, fig.width = 10, fig.height= 6, fig.cap = "Greater silver smelt in 5a and 14. Projected catches, spawning stock biomass and fishing mortality for select target harvest rates. Red and black dashed horizontal lines represent the limit and pa reference points respectively for SSB and F. "}

rates <- c(0.26,0.30,0.34,0.38,0.42)
...
#  bind_rows(.id='rate') %>% 
  ...
  left_join(res$mat.ssb %>% 
      #  bind_rows(.id='rate') %>% 
      filter(round(harvest_rate,2) %in% rates,year < 2050, trial==113) %>% 
      mutate(ssb = number*mean_weight) %>% 
      mutate(rep = rep(1:10,each = 60)) %>%
      filter(rep == 1) %>% 
      group_by(year,harvest_rate) %>%
  ...
  left_join(res$mat.ssb %>% 
      #  bind_rows(.id='rate') %>% 
      filter(round(harvest_rate,2) %in% rates,year < 2050, trial==215) %>% 
      mutate(ssb = number*mean_weight) %>% 
      mutate(rep = rep(1:10,each = 60)) %>%
      filter(rep == 1) %>% 
      group_by(year,harvest_rate) %>%
    ...
    left_join(res$mat.ssb %>% 
      #  bind_rows(.id='rate') %>% 
      filter(round(harvest_rate,2) %in% rates,year < 2050, trial==317) %>% 
      mutate(ssb = number*mean_weight) %>% 
       mutate(rep = rep(1:10,each = 60)) %>%
      filter(rep == 1) %>% 
     group_by(year,harvest_rate) %>%
      ...
      left_join(res$mat.ssb %>% 
      #  bind_rows(.id='rate') %>% 
      filter(round(harvest_rate,2) %in% rates,year < 2050, trial==519) %>% 
      mutate(ssb = number*mean_weight) %>% 
      mutate(rep = rep(1:10,each = 60)) %>%
      filter(rep == 1) %>% 
      group_by(year,harvest_rate) %>%
      ...
      left_join(res$mat.ssb %>% 
      #  bind_rows(.id='rate') %>% 
      filter(round(harvest_rate, 2) %in% rates,year < 2050, trial==921) %>% 
      mutate(ssb = number*mean_weight) %>% 
      mutate(rep = rep(1:10,each = 60)) %>%
      filter(rep == 1) %>% 
      group_by(year,harvest_rate) %>%
...
ssb_progn.plot <- 
  ...
  facet_wrap(~harvest_rate,ncol=1) +
  theme_light() + 
...
catch_dat <-
  ...
  bind_rows(fit$res.by.year %>% 
              ...
              right_join(expand.grid(year=1990:2019,
                                    harvest_rate=rates)))%>% 
  left_join(res$catch.lw %>% 
      #  bind_rows(.id='rate') %>% 
      filter(round(harvest_rate,2) %in% rates,year < 2050, trial==113) %>% 
    mutate(harvest_rate = round(harvest_rate,2)) %>% 
      mutate(rep = rep(1:10,each = 60*4)) %>%
      filter(rep == 1) %>% 
    group_by(year,harvest_rate) %>%
...
The proposed target harvest rate, of `r round(ftarget,2)` illustrated in fig. Figure 14, is considered consistent with the ICES MSY approach. The expected 5% and 95% percentiles of true harvest rates, when setting catches according __, are ... and ... respectively, as illustrated by figure FIGURE NEED TO UPDATE.
...
Precautionary approach | $B_{lim}$         |`r round(blim,2)` kt      | $B_{pa}/e^{1.645*sigma}$ where $\sigma=0.2$
                -      | $B_{pa}$          | `r round(bpa,2)` kt      | SSB(2003), corresponding to $B_{loss}$ 
                 -     | $F_{lim}$         |  `r round(f.lim,2)`      | $F$ corresponding to 50% long-term probability of SSB > $B_{lim}$ 
                  -    | $F_{pa}$          |   `r round(f.pa,2)`      | $F_{lim}/e^{1.645*sigma}$ where $\sigma=0.0.25$
-----------------------|-------------------|-----------------|----------------
Advisory rule          | $F_{target}$      |  `r round(ftarget,2)`    | $F$ such that $F\leq F_{msy}$, $F\leq F_{pa}$, and $F\leq F_{0.05}$, long-term yield is consistent with MSY while leading to high stock biomass 
