...
library(mfdb)
mdb <- mfdb('Iceland')
setwd('~/Documents/gadget-models/06-ling/99-docs/pre/')
...
<<echo=FALSE,results='asis',fig.width=10, fig.height=5,cache=TRUE,message=FALSE,warning=FALSE>>=
 mfdb_dplyr_sample(mdb) %>%
   ...
   collect(n=Inf) %>%
   ungroup() %>%
 tidyr::gather(col,val,-c(year,sampling_type)) %>%
   ggplot(aes(year,val)) + geom_bar(stat='identity',fill='#0076a9') +
   facet_wrap(~sampling_type+col,scale='free_y',ncol=4)+
   theme_light() +
     geom_label(data= data_frame(col=rep(c('n','ntow'),4),
                                 sampling_type=rep(c('lln','bmt','gil','igfs'),each=2),
                                 label=c('Longline \n (n measured)','Longline\n (n tows)',
...
#  geom_text(x=95,y=0.04,angle=90,aes(label=paste0('n = ',round(total.catch))),size=3) +
  ...
  geom_text(data=data_frame(year=1985:2016),
            aes(label=year),x=78,y=0.07,size=3)
@
}
...
#  geom_text(x=140,y=0.07,angle=90,aes(label=paste0('n = ',round(total.catch)))) +
  xlab('Length') + ylab('Proportion') +
  theme(strip.background = element_blank(),strip.text = element_blank()) +
  geom_label(data=expand.grid(year=2000:2016,step=1:4) %>% dplyr::mutate(label=paste(year,step,sep=', ')),
...
gridExtra::grid.arrange(bio.plot,ssb.plot,F.plot,rec.plot,catch.plot,cv.plot,ncol=3)
...
\frame{
\frametitle{Derivation of reference points}
\bi The proposed form for managment rule is $C_y = H B_{75cm^+}$
\item No evidence of impaired recruitment and fishing mortality is considered to have been low
\item $B_{loss} = 9.93 kt$ is suggested as a candidate for $B_{pa}$
\item $B_{lim} = B_{pa}/1.4 = 7.09$
\item $H_{lim}$ set as the harvest rate that has 50\% chance of SSB being at $B_{lim}$ via stochastic simulation without assessment error
\item Variability in recruitment is based on a block bootstrap of estimated recruitment, block size of 6 consequtive years.
\item $F_{lim}$, $F_{pa}$ and $H_{pa}$ are estimated based on the estimate of $H_{lim}$
\item $H_{msy}$ is based on simulation with assessment error (lognormal with $\rho = 0.8$, and $\sigma$ as the CV of $B_{75cm^+}$)
\ei
}
...
\frame{
\frametitle{Proposed management plan}
