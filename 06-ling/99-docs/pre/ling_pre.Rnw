...
library(tidyverse)
load('../../03-three_fleets/WGTS/WGTS.Rdata')
fit <- out
@
\bi Survey indices from the Icelandic groundfish survey from 1984:
  \bi si.20-50 
  \item si.50-70, split into two sub-groups
  \item si.70-160, split into 4 sub-groups
\ei
