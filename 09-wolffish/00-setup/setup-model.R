...
init.sigma <- 
  mfdb_dplyr_sample(mdb) %>% 
  dplyr::filter(species == defaults$species,age >0,!is.na(length), sex == 'F', year > 2001)  %>% #per Ásgeir's suggestion of data 
...
wolf.imm <- 
  gadgetstock('wolfimm',gd$dir,missingOkay = TRUE) %>%
  gadget_update('stock',
                minage = 1,
                maxage = 23,
                minlength = 5,
  ...
  gadget_update('doesgrow', ## note to self the order of these parameters make difference
                growthparameters=c(linf='#wolf.Linf', 
                                   k=to.gadget.formulae(quote(0.001*wolfimm.k)),
                                   alpha = '#wolfimm.walpha',
                                   beta = '#wolfimm.wbeta'),
                beta = to.gadget.formulae(quote(10*wolf.bbin)),
                maxlengthgroupgrowth = 4) %>% 
  gadget_update('initialconditions',
                normalparam = data_frame(age = .[[1]]$minage:.[[1]]$maxage,
                                         area = 1,
                                         age.factor = parse(text=sprintf('exp(-1*(wolfimm.M+wolf.init.F)*%1$s)*exp(wolfimm.init.%1$s)',age)) %>% 
                                           map(to.gadget.formulae) %>% 
                                           unlist(),   
                                         area.factor = '#wolfimm.init.scalar',
                                         mean = von_b_formula(age,linf='wolf.Linf',k='wolfimm.k',recl='wolf.recl'),
  ...
  gadget_update('doesmature', 
                ...
                coefficients = '( * 0.001 #wolf.mat1) #wolf.mat2 0 0') %>% 
  ...
  gadget_update('doesrenew',
                normalparam = data_frame(year = year_range,
                                         ...
                                         area = 1,
                                         age = .[[1]]$minage,
                                         number = parse(text=sprintf('wolf.rec.scalar*exp(wolf.rec.%s)',year)) %>% 
                                           map(to.gadget.formulae) %>% 
                                           unlist(),
                                         mean = von_b_formula(age,linf='wolf.Linf',k='wolfimm.k',recl='wolf.recl'),
                                         stddev = '#wolf.rec.sd',
...
wolf.mat <-
  gadgetstock('wolfmat',gd$dir,missingOkay = TRUE) %>%
  gadget_update('stock',
                minage = 5,
                maxage = 23,
                minlength = 5,
  ...
  gadget_update('initialconditions',
                normalparam = data_frame(age = .[[1]]$minage:.[[1]]$maxage,
                                         area = 1,
                                         age.factor = parse(text=sprintf('exp(-1*(wolfmat.M+wolf.init.F)*%1$s)*exp(wolfmat.init.%1$s)',age)) %>% 
                                           map(to.gadget.formulae) %>% 
                                           unlist(),
                                         area.factor = '#wolfmat.init.scalar',
                                         mean = von_b_formula(age,linf='wolf.Linf',k='wolfmat.k.2',recl='wolf.recl'),
                                         stddev = init.sigma$ms[age],
                                         alpha = '#wolfmat.walpha',
                                         beta = '#wolfmat.wbeta')) %>% 
  ## does"something" updates should also allow for other names, e.g. doesrenew -> recruitment etc..
  gadget_update('refweight',
                data=data_frame(length=seq(.[[1]]$minlength,.[[1]]$maxlength,.[[1]]$dl),
...
read.gadget.parameters(sprintf('%s/params.out',gd$dir)) %>% 
  init_guess('rec.[0-9]',log(1),log(0.001),log(1000),1) %>%
  init_guess('init.[0-9]',log(1),log(0.001),log(1000),1) %>%
  init_guess('recl',12,1,20,1) %>% 
  init_guess('rec.sd',5, 1, 20,1) %>% 
  init_guess('Linf',160, 100, 200,1) %>% 
  init_guess('k$',90, 20, 150,1) %>% 
  init_guess('k.[0-9]',90, 20, 100,1) %>% 
  ...
  init_guess('mat2',mat.l50$l50,0.60*mat.l50$l50,1.25*mat.l50$l50,1) %>% 
