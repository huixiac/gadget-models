...
path <- '06-ling/11-age12plus/'
mainfile <- 'main'
num.years <- 300
num.iter <- 10
params.file <- 'WGTS_v2/params.final'
pre <- 'PRE'
imm.stock <- 'lingimm'
mat.stock <- 'lingmat'
...
load('06-ling/99-docs/bootfit.Rdata')
## read function
...
rep.params <- function(params,year_range = 2016){ 
    ...
    purrr::map(function(x){
        block.rec <-
            ...
            purrr::map(function(x) {
                schedule %>%
                    filter(step==1) %>% 
                    mutate(number=(tseries::tsbootstrap(x$number,type='block',nb=2*num.years/30,b=block.size) %>% as.numeric())[1:n()])
            }) %>%
            bind_rows(.id='stock') %>% 
...
tmp <- 
  params.pre %>% 
  mutate(repl = ceiling(1:n()/686)) %>% 
  split(.$repl) %>%
  parallel::mclapply(function(x) {
      dplyr::select(x,switch,value) %>% 
...
resnotrigger <- mclapply(tmp2,run.func,mc.cores = detectCores(logical = TRUE))
...
resbtrigger <- mclapply(tmp2,run.func,mc.cores = detectCores(logical = TRUE),btrigger=TRUE)
                                        ...
                                        #reruns <- as.character(c(2,3,5,7:9,11,12,15,24,40:45,47:50,53,54,51,63:64)/100)
...
save(res0,file='res0_ling.Rdata')
save(res,file='res_ling.Rdata')
...
save(res20,res_by_year0,res_adv0,file='ling_adv_res0.Rdata')
